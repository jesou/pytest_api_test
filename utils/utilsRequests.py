...
class RequestConfig:
    ...
    def _request_auth(self):
        response_data = HTTPBasicAuth(self.user, self.password) if self.user is not None or \
                                                                    self.password is not None else None
        ...
        if method.upper() == 'GET':
            if '{' not in params:
                if params != '' or params is not None:
                    url = url + params
                response_data = requests.get(url=url, headers=header, auth=HTTPBasicAuth(self.user, self.password) if
                                             self.user is not None or self.password is not None else None)
                ...
                response_data = requests.get(url=url, headers=header, params=params, auth=
                                             HTTPBasicAuth(self.user, self.password) if self.user is not None or
                                             self.password is not None else None)
                self.logger.info('响应码: %r, 响应结果: %s' % (response_data.status_code, response_data.text))
                allureReportConfig.step("响应内容", "responseBody", response_data.text)
                return response_data
            ...
            if postType == 'params':
                # url 拼params
                url = url + str(params)
                response_data = requests.post(url=url, headers=header, auth=HTTPBasicAuth(self.user, self.password) if
                                              self.user is not None or self.password is not None else None)
                self.logger.info('响应码: %r, 响应结果: %s' % (response_data.status_code, response_data.text))
                allureReportConfig.step("响应内容", "responseBody", response_data.text)
                return response_data
                ...
                response_data = requests.post(url=url, headers=header, data=params,
                                              auth=HTTPBasicAuth(self.user, self.password) if self.user is not None or
                                              self.password is not None else None)
                self.logger.info('响应码: %r, 响应结果: %s' % (response_data.status_code, response_data.text))
                allureReportConfig.step("响应内容", "responseBody", response_data.text)
            return response_data
            ...
            try:
                response_data = requests.request(method.lower(), url=url, headers=header,
                                                 data=params, auth=HTTPBasicAuth(self.user, self.password) if self.user
                                                 is not None or self.password is not None else None)
